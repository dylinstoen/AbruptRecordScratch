...
namespace _Project.Scripts.Gameplay {
    public class KeyedPool<TEnum, TInstance> : MonoBehaviour where TEnum : unmanaged, Enum where TInstance : Component, IPoolKeyed<TEnum> {
        [Serializable]
        public class Config {
            public TEnum type;
        ...
        private readonly Dictionary<TEnum, Config> _typeToConfig = new();
        private readonly Dictionary<TEnum, Queue<TInstance>> _available = new();
        private readonly Dictionary<TEnum, Queue<TInstance>> _activeOldest = new();
        ...
        public TInstance Rent(TEnum type) {
        ...
        private TInstance ProcessFull(Config config, TEnum type) {
        ...
        private void Prewarm(Config c, TEnum type, int count) {
            if (count <= 0) return;
        ...
        private void ReturnToPool(Component component, TEnum type) {
